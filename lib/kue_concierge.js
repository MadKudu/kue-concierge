...
KueConcierge.prototype.clearComplete = function() {
	var deferred = Q.defer();
	var instance = this;
	this.kue.Job.rangeByState('complete', 0, instance.batchSize, 'asc', function (err,jobs) {
		if (err) {
			deferred.reject(err);
		}
		jobs.forEach(function (job) {
...
KueConcierge.prototype.clearFailed = function() {
	var deferred = Q.defer();
	var instance = this;
	this.kue.Job.rangeByState('failed', 0, instance.batchSize, 'asc', function (err,jobs) {
		if (err) {
			deferred.reject(err);
		}
		jobs.forEach(function (job) {
